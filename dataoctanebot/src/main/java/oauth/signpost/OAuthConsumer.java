...
public interface OAuthConsumer extends Serializable {
	 ...
	 * {@link SigningStrategy} will then take care of writing them to the correct
	 * part of the request before it is sent. This is useful if you want to pre-set
	 * custom OAuth parameters. Note that these parameters are expected to already
	 * be percent encoded -- they will be simply merged as-is. <b>BE CAREFUL WITH
	 * THIS METHOD! Your service provider may decide to ignore any non-standard
	 * OAuth params when computing the signature.</b>
	 * 
	...
	/**
	 * <p>
	 * Causes the consumer to always include the oauth_token parameter to be sent,
	 * even if blank. If you're seeing 401s during calls to
	 ...
	 * required OAuth parameters) to it. Where these parameters are written depends
	 * on the current {@link SigningStrategy}.
	 * 
	 * @param request
	 *            the request to sign
	 ...
	 * implementation must ensure that only those request types are passed which it
	 * supports.
	 * 
	 * @param request
	 *            the request to sign
	 ...
	 * required for message signing. The assumed HTTP method is GET. Essentially,
	 * this is equivalent to signing an HTTP GET request, but it can be useful if
	 * your application requires clickable links to protected resources, i.e. when
	 * your application does not have access to the actual request that is being
	 * sent.
	 * </p>
	 * 
	 * @param url
	...
	/**
	 * Returns all parameters collected from the HTTP request during message signing
	 * (this means the return value may be NULL before a call to {@link #sign}),
	 * plus all required OAuth parameters that were added because the request didn't
	 * contain them beforehand. In other words, this is the exact set of parameters
	 * that were used for creating the message signature.
	 * 
