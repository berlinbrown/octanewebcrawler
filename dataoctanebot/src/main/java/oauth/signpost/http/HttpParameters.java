...
public class HttpParameters implements Map<String, SortedSet<String>>, Serializable {
	...
	/**
	 * Returns the first value from the set of all values for the given parameter
	 * name. If the key passed to this method contains special characters, you MUST
	 * first percent encode it using {@link OAuth#percentEncode(String)}, otherwise
	 * the lookup will fail (that's because upon storing values in this map, keys
	 * get percent-encoded).
	 * 
	 * @param key
	 *            the parameter name (must be percent encoded if it contains unsafe
	 ...
	 *            the parameter name
	 * @param percentEncode
	 *            whether key should be percent encoded before being used with the
	 *            map
	 * @return the query string
	 */
