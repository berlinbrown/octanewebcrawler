...
public abstract class AbstractOAuthProvider implements OAuthProvider {
	...
	public synchronized String retrieveRequestToken(OAuthConsumer consumer, String callbackUrl,
			String... customOAuthParams) throws OAuthMessageSignerException, OAuthNotAuthorizedException,
	...
	public synchronized void retrieveAccessToken(OAuthConsumer consumer, String oauthVerifier,
		...
		if (consumer.getToken() == null || consumer.getTokenSecret() == null) {
			throw new OAuthExpectationFailedException("Authorized request token or token secret not set. "
	...
	/**
	 * <p>
	 * Implemented by subclasses. The responsibility of this method is to contact
	 * the service provider at the given endpoint URL and fetch a request or access
	 * token. What kind of token is retrieved solely depends on the URL being used.
	 * </p>
	 * <p>
	 ...
	 * {@link OAuth#OAUTH_TOKEN} and {@link OAuth#OAUTH_TOKEN_SECRET} set by calling
	 * {@link OAuthConsumer#setTokenWithSecret(String, String)}</li>
	 * <li>{@link #getResponseParameters()} must return the set of query parameters
	 * served by the service provider in the token response, with all OAuth specific
	 * parameters being removed</li>
	 * </ul>
	 * </p>
	 * 
	 * @param consumer
		...
		switch (statusCode) {
			...
			default :
				throw new OAuthCommunicationException(
						"Service provider responded in error: " + statusCode + " (" + response.getReasonPhrase() + ")",
	 ...
	 * Use this to do any cleanup / resource freeing.
	 * 
	 * @param request
	...
	/**
	 * Returns a single query parameter as served by the service provider in a token
	 * reply. You must call {@link #setResponseParameters} with the set of
