...
public class OAuth {
	...
	/**
	 * Pass this value as the callback "url" upon retrieving a request token if your
	 * application cannot receive callbacks (e.g. because it's a desktop app). This
	 * will tell the service provider that verification happens out-of-band, which
	 * basically means that it will generate a PIN code (the OAuth verifier) and
	 * display that to your user. You must obtain this code from your user and pass
	 * it to {@link OAuthProvider#retrieveAccessToken(OAuthConsumer, String)} in
	 * order to complete the token handshake.
	 */
	...
	/**
	 * Construct a x-www-form-urlencoded document containing the given sequence of
	...
	/**
	 * Builds an OAuth header from the given list of header fields. All parameters
	...
	/**
	 * Helper method to concatenate a parameter and its value to a pair that can be
	 * used in an HTTP header. This method percent encodes both parts before joining
	 * them.
	 * 
	 * @param name
