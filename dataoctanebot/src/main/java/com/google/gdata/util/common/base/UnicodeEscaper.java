...
public abstract class UnicodeEscaper implements Escaper {
	 ...
	 * valid <a href="http://en.wikipedia.org/wiki/UTF-16">UTF-16</a> before calling
	 * this method or use an escaped {@link Appendable} (as returned by
	...
	/**
	 * Returns an {@code Appendable} instance which automatically escapes all text
	 * appended to it before passing the resulting text to an underlying
	 * {@code Appendable}.
	 *
	 * <p>
	 * Unlike {@link #escape(String)} it is permitted to append arbitrarily split
	 * input to this Appendable, including input that is split over a surrogate
	 * pair. In this case the pending high surrogate character will not be processed
	 * until the corresponding low surrogate is appended. This means that a trailing
	 * high surrogate character at the end of the input cannot be detected and will
	 * be silently ignored. This is unavoidable since the Appendable interface has
	 * no {@code close()} method, and it is impossible to determine when the last
	 * characters have been appended.
	 *
	 * <p>
	 * The methods of the returned object will propagate any exceptions thrown by
	 * the underlying {@code Appendable}.
	 *
	 * <p>
	 * For well formed <a href="http://en.wikipedia.org/wiki/UTF-16">UTF-16</a> the
	 * escaping behavior is identical to that of {@link #escape(String)} and the
	 * following code is equivalent to (but much slower than)
	 ...
	 * @return an {@code Appendable} which passes text to {@code out} after escaping
	 *         it
	 * @throws NullPointerException
	 *             if {@code out} is null
	 * @throws IllegalArgumentException
